...
#define	SERVO_CENTER_POSITION 2030
#define SERVO_CW_LIMIT 2330
#define SERVO_CCW_LIMIT 1720
/*
...
int main(void)
{
	 ...
	 * Center Drive Servos
	 */
	Dynamixel_Goal_Position_Write(SERVO_ID_BACK_WHEEL,SERVO_CENTER_POSITION,0x0000);
	Dynamixel_Goal_Position_Write(SERVO_ID_FRONT_WHEEL,SERVO_CENTER_POSITION,0x0000);
	/*		initialize AXIGpio 	*/
	GpioInit();
...
void PWM_write(u8 data)
{
	...
	MD03ADRIVER_mWriteReg(PWM_BASE,0,data);
	MD03ADRIVER_mWriteReg(PWM_BASE,4,data);
...
void PWM_reset(void)
{
	 MD03ADRIVER_mWriteReg(PWM_BASE,0,0x00000000);
	 MD03ADRIVER_mWriteReg(PWM_BASE,4,0x00000000);
	 MD03ADRIVER_mWriteReg(PWM_BASE,8,0x00000001);
	 usleep(1);
	 MD03ADRIVER_mWriteReg(PWM_BASE,8,0x00000000);
}
...
void process_data(char id,char *data)
{
	...
	case 'B':			// Set Dynamixel Position
		...
		if (servoPosition >SERVO_CW_LIMIT)
			servoPosition = SERVO_CW_LIMIT;
		if (servoPosition < SERVO_CCW_LIMIT)
			servoPosition = SERVO_CCW_LIMIT;
		Dynamixel_Goal_Position_Write(SERVO_ID_BACK_WHEEL,servoPosition,0x0000);
		Dynamixel_Goal_Position_Write(SERVO_ID_FRONT_WHEEL,servoPosition,0x0000);
		break;
	default:
		break;
	}
